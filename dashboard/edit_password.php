					                    ...
					                    <?php
                    							if(isset($_POST['submit'])){
                    								...
                    								if($confirm_password==$new_password){
                    									$sql_check=mysqli_query($con,"SELECT * FROM `user` WHERE `email`='$userid' AND `password`='$old_password'");
                    									$num=mysqli_num_rows($sql_check);
                    									if($num>=1){
                    										$up=mysqli_query($con,"UPDATE `user` SET  `password`='$new_password' WHERE `email`='$userid'");
                    										...
                    										echo '<script>alert("New Password and Conform Password Should Match!!!.");window.location.assign("edit_password.php");</script>';
                    								}
                    							}
                                           ?>
                </div>
